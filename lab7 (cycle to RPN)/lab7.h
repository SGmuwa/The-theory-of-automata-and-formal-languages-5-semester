...
int lab7_putElse(ArrayList outList, string_t * buffer)
{
#define LAB7_SAFE(ACT, CODE) if(ACT) return CODE
	...
	{
		string_t a, b;
		LAB7_SAFE(ArrayList_get(outList, i, &a), 3);
		if (string_equal(STRING_STATIC0("if"), a))
		{
			LAB7_SAFE(ArrayList_get(outList, i - 1, &b), 4);
			if (string_equal(searchAddress, b))
			{
				LAB7_SAFE(lab7_putSizetToEndString(buffer, outList->length, &searchAddress), 5);
				LAB7_SAFE(ArrayList_set(outList, i - 1, &searchAddress), 6);
				return 0;
			}
		}
	}
...
int lab7(string_t * output, string_t input)
{
	...
	char previous = '\0';
#define LAB7_SAFE(ACT, CODE) if(ACT) { Stack_free(stk); free(oldIn); ArrayList_free(outList); string_free(bufferForOutput); return CODE; }
	...
	{ // Пока мы ещё имеем входную строку.
		...
		{ // Найдена функция
			...
			{
				LAB7_SAFE(lab7_putElse(outList, &bufferForOutput), 6);
			}
			else
			{
